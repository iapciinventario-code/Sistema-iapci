async function guardarDatosUsuario(datos) {
    try {
        // Intentar guardar en Firebase Firestore
        await db.collection("iapci_usuarios").add(datos);
        console.log("Guardado exitoso en la nube");
    } catch (error) {
        console.warn("Fallo la conexión con la nube, guardando localmente...", error);
        // Respaldo de emergencia en localStorage
        let locales = JSON.parse(localStorage.getItem("iapci_usuarios") || "[]");
        locales.push(datos);
...
const firebaseConfig = {
  ...
  databaseURL: "[https://sistema-iapci-default-rtdb.firebaseio.com](https://sistema-iapci-default-rtdb.firebaseio.com)",
